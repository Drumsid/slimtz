...
$app->get('/', function ($request, $response, array $args) {
    return $response->write('Welcome to Simple API!');
});

$app->get('/users', function ($request, $response) {
    if (count(json_decode(LIST_OF_USERS, true)) > 0) {
        return $response->withHeader('Content-type', 'application/json')->write(LIST_OF_USERS)->withStatus(200);
    }
    return $response->write('Not one user was found, time itself to add a new user!')->withStatus(400);
});
